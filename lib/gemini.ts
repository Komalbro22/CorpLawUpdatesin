...
export async function getEmbedding(text: string): Promise<number[]> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Gemini embedding fetch failed: ${response.statusText} ${JSON.stringify(errorData)}`)
  }
...
`
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(`Gemini variable extraction failed: ${response.statusText} ${JSON.stringify(errorData)}`)
  }
...
`
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}))
    throw new Error(`Gemini custom draft generation failed: ${response.statusText} ${JSON.stringify(errorData)}`)
  }
