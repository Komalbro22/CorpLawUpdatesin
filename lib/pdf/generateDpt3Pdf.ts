...
export interface Dpt3PdfData {
  ...
  filingPurpose: 'deposits' | 'exempted' | 'both' | 'nil'
  selectedFY: string
...
export function generateDpt3Pdf(data: Dpt3PdfData): jsPDF {
  const doc = new jsPDF()
  ...
  if (data.filingPurpose === 'deposits') {
    ...
    purposeLabel = 'Nil Return (Best Practice — No Deposits or Exempted Receipts Outstanding)'
  }
  ...
  const afterFees = (doc as any).lastAutoTable.finalY + 8
  doc.setFontSize(10)
  doc.setTextColor(navy[0], navy[1], navy[2])
  doc.setFont('helvetica', 'bold')
  ...
  const penaltyRows = [
    [
      ...
      'Company: ₹5,000 + ₹500/day | Officers: ₹5,000 + ₹500/day',
      `₹ ${data.rule21Penalty.toLocaleString('en-IN')}`
    ],
    [
      'Section 76A Deposit Contravention',
      'Applies ONLY if unauthorized public deposits are accepted',
  ...
  // Footer & Disclaimers
  const afterPenalties = (doc as any).lastAutoTable.finalY + 8
  doc.setFontSize(7)
  doc.setTextColor(gray[0], gray[1], gray[2])
  doc.setFont('helvetica', 'italic')
  doc.text(
    'LEGAL NOTICE: This assessment is automatically generated by CorpLawUpdates.in for professional advisory and planning purposes.',
    14,
    afterPenalties
  )
  doc.text(
