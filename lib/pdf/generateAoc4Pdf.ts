...
import { Aoc4ComplianceCalculationResult } from '@/lib/rule-engine/aoc4-engine'
import {
  cleanPdfText,
  cleanTableData,
...
export function generateAoc4Pdf(result: Aoc4ComplianceCalculationResult, companyName?: string): jsPDF {
  const doc = new jsPDF()
  ...
  const startY = renderDocumentHeader(doc, {
    ...
    subtitle: 'Section 137(1), Companies Act, 2013 & Rule 12, Companies (Accounts) Rules, 2014',
    dateLabel: 'Report Date'
  })

  // 1. Filing & Entity Parameters Table
  const parameterRows: any[] = [
    ['Form Number & Title', `${result.metadata.formCode} - ${result.metadata.formName}`, 'Section 137(1), Companies Act 2013 & Rule 12'],
    ['Company Name', companyName ? companyName.toUpperCase() : 'Not Specified (Generic Estimation)', 'As registered on MCA21 portal'],
    ['Financial Year', result.metadata.financialYear, 'Relevant annual financial statements period'],
    ...
    ['AGM / Filing Timeline', result.metadata.companyClassification.includes('OPC') ? 'OPC: 180 days from FY closure' : `${result.metadata.agmType === 'first' ? 'First AGM' : 'Subsequent AGM'} (${result.metadata.agmStatus})`, result.metadata.companyClassification.includes('OPC') ? 'Sec 137(1) third proviso (27th September)' : 'Sec 137(1): 30 days from AGM'],
    ['Statutory Due Date', result.metadata.statutoryDueDate, `Statutory deadline (${result.metadata.daysDelayed > 0 ? result.metadata.daysDelayed + ' day(s) overdue' : 'Compliant'})`],
    ...
    ['Cash Flow Statement', result.cashFlowExemption.isExempt ? 'EXEMPT FROM ATTACHMENT' : 'MANDATORY ATTACHMENT', result.cashFlowExemption.note],
    ['XBRL Applicability', result.xbrlAssessment.mustFileXbrl ? 'MANDATORY XBRL FILING' : 'STANDARD NON-XBRL AOC-4', result.xbrlAssessment.reason],
  ...
  // 3. Financial Panel 2: Indicative Section 137(3) Penalty Exposure
  const finalYPortal = (doc as any).lastAutoTable.finalY + 4

  doc.setFontSize(9.5)
  doc.setTextColor(PDF_PALETTE.navy[0], PDF_PALETTE.navy[1], PDF_PALETTE.navy[2])
  doc.setFont('helvetica', 'bold')
  doc.text('2. Indicative Section 137(3) Statutory Penalty Exposure (Adjudication Required)', 14, finalYPortal)
  ...
  const disclaimer = 
    "LEGAL DISCLAIMER & STATUTORY NOTICE: This report is generated automatically by CorpLawUpdates.in from user-entered parameters and applicable rule data. It is provided for informational and estimation purposes only and does not constitute an audit, certification, legal opinion, or official MCA document. The MCA portal remains the final authority for the fee charged at filing. Section 137(3) penalties are not collected via MCA21 e-Challan; they represent potential statutory exposure if adjudication proceedings are initiated by the Registrar of Companies under Section 454."
