...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  mca_forms: 'bg-rose-500/10 border-rose-500/20 text-rose-600',
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="admin-card p-6 flex items-center gap-4 col-span-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-base font-bold text-slate-900 mt-1 truncate" title={popularTemplate?.name}>
              {isLoading ? (
                <Loader2 className="w-5 h-5 animate-spin text-slate-500" />
              ) : popularTemplate ? (
      ...
      <div className="admin-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/60 border-b border-white/60 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                <th
            ...
            <tbody className="divide-y divide-slate-200/50/60">
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="p-12 text-center bg-slate-100/10">
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center bg-slate-100/10">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span className="text-base font-bold text-slate-900">No Templates Found</span>
                  ...
                  <tr key={t.id} className="hover:bg-slate-100/50 transition-colors group">
                    <td className="p-4 max-w-sm">
                      <div className="flex items-start gap-2">
                        <div>
                          <h4 className="font-semibold text-slate-800 text-sm group-hover:text-amber-500 transition-colors">
                            {t.name}
                          </h4>
                    ...
                    <td className="p-4">
                      <span
                        className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold border ${
                          CATEGORY_COLORS[t.category] || 'bg-slate-200/50 border-slate-700 text-slate-700'
                        }`}
                      >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] text-slate-500">generations</span>
                      </div>
                    </td>
                    <td className="p-4">
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold transition-all ${
                          t.is_active
                            ...
                            : 'bg-slate-100 text-slate-500 border border-slate-700'
                        }`}
                      >
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1.5 opacity-80 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          className="p-2 text-slate-500 hover:text-blue-400 hover:bg-slate-100 rounded-lg transition-colors"
                          title="Edit Template"
                        >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="border-t border-white/60 pt-6">
                  ...
                  {modalFields.length === 0 ? (
                    <div className="bg-slate-50/50 border border-dashed border-white/60 rounded-xl p-6 text-center text-slate-500 text-xs">
                    ...
                    <div className="space-y-4">
                      {modalFields.map((field, idx) => (
                        <div
                          key={field.id}
                          className="p-4 bg-slate-50 border border-white/60 rounded-xl relative space-y-3"
                        >
                          <button
                            type="button"
                            onClick={() => handleRemoveField(idx)}
                            className="absolute top-3 right-3 text-slate-500 hover:text-rose-600 transition-colors"
                            title="Remove Field"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            ...
                            <div>
                              ...
                              >
                                ...
                                <option className="bg-slate-50 text-slate-900" value="date">Calendar Date</option>
                                <option className="bg-slate-50 text-slate-900" value="select">Dropdown Select</option>
                              </select>
                            </div>
...
# RESOLVED THAT Mr. {director_name} be appointed as director..."
              ...
              {/* Modal Actions */}
              <div className="p-6 border-t border-white/60 bg-slate-50 flex justify-end gap-3.5">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 border border-white/60 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-100 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2 bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-slate-900 rounded-xl text-sm font-bold shadow-md transition-colors flex items-center gap-2"
                >
                  {isSubmitting ? (
                    <>
