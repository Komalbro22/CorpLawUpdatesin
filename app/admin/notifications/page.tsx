...
import { Bell, Send, CheckCircle2, AlertCircle, RefreshCw, Users, ShieldCheck, Zap, History, Sparkles, ExternalLink } from 'lucide-react'
...
const TEMPLATES = [
  {
    label: 'MCA Notification',
    title: 'New MCA Circular: Statutory Compliance Deadline Amendment',
    body: 'MCA issues important guidelines regarding annual ROC return filings & Director KYC requirements.',
    category: 'mca',
    url: '/category/mca',
  },
  {
    label: 'SEBI Circular',
    title: 'SEBI LODR Disclosure Norms Updated',
    body: 'SEBI introduces amended compliance timelines for listed entity quarterly filings.',
    category: 'sebi',
    url: '/category/sebi',
  },
  {
    label: 'RBI Update',
    title: 'RBI Notification: Banking Regulatory Directives',
    body: 'Reserve Bank of India updates cross-border trade & commercial lending reference rates.',
    category: 'rbi',
    url: '/category/rbi',
  },
  {
    label: 'NCLT Order',
    title: 'Key NCLT Judgment: Corporate Restructuring Benchmark',
    body: 'National Company Law Tribunal rules on insolvency procedure priorities & scheme sanctioning.',
    category: 'nclt',
    url: '/category/nclt',
  },
]
    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-semibold rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors self-start sm:self-auto"
        >
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-md">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
          <h2 className="text-lg font-bold font-heading text-navy dark:text-white">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {TEMPLATES.map((tpl) => (
              <button
                ...
                className="text-[11px] font-semibold px-2.5 py-1 rounded-lg bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 hover:bg-amber-100 transition-colors"
              >
                {tpl.label}
              </button>
            ))}
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-md">
        <h2 className="text-lg font-bold font-heading text-navy dark:text-white flex items-center gap-2 mb-4">
          <History className="w-5 h-5 text-amber-500" /> Recent Push Broadcast History
        </h2>

        {recentLogs.length === 0 ? (
          <div className="py-8 text-center text-xs text-slate-400">
            No broadcast notifications logged yet. Send a push notification above to track dispatch logs.
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs whitespace-nowrap">
              <thead className="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">Sent / Targeted</th>
                  <th className="px-4 py-3 font-semibold">Pruned (410/404)</th>
                ...
                {recentLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-semibold text-navy dark:text-white">{log.title}</div>
                      {log.url && (
                        <span className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                          {log.url} <ExternalLink className="w-2.5 h-2.5" />
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 font-medium">
                      <span className="text-emerald-600 font-bold">{log.sent_count}</span> / {log.targeted_count}
                    </td>
                    <td className="px-4 py-3 text-slate-500">
                      {log.pruned_count > 0 ? (
                        <span className="text-red-500 font-bold">{log.pruned_count}</span>
                      ) : (
                        '0'
                      )}
                    </td>
