    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h2 className="font-bold text-navy mb-4">🏆 Top Articles by Views</h2>
            ...
            <div className="space-y-3">
              {topArticles.map((article, i) => (
                <div key={article.slug} className="flex items-start gap-3">
                  <span className="text-lg font-bold text-slate-300 w-6 flex-shrink-0">
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      target="_blank"
                      className="text-sm font-medium text-navy hover:text-amber-600 line-clamp-2"
                    >
                      {article.title}
                    </Link>
        ...
        {/* CATEGORY BREAKDOWN */}
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h2 className="font-bold text-navy mb-4">📊 Articles by Category</h2>
                    ...
                    <div key={cat}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-slate-500">
                          {count} article{count !== 1 ? 's' : ''} · {pct}%
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full ${categoryColors[cat] || 'bg-slate-400'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
        ...
        {/* RECENT SUBSCRIBERS */}
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h2 className="font-bold text-navy mb-4">📧 Recent Subscribers</h2>
          {recentSubscribers.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              <Link
                ...
                className="block text-center text-sm text-amber-600 hover:text-amber-700 font-medium mt-3"
              >
