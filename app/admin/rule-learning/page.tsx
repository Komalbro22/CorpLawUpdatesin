...
export default function RuleLearningDashboard() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto px-4 py-6 text-slate-100">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.06] pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white font-heading">AI Rule Learning Queue</h1>
            <p className="text-xs text-slate-400">Review, configure, and approve parameterized clause templates gathered from manual user edits.</p>
          </div>
        </div>
        <button
          onClick={fetchQueue}
          disabled={loading}
          className="flex items-center gap-2 rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-xs font-semibold hover:bg-white/10 hover:text-white transition-all disabled:opacity-50"
        >
      ...
      {loading && items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white/[0.01] rounded-2xl border border-white/[0.04] border-dashed">
          ...
          <p className="text-slate-400 text-sm">Loading learning queue...</p>
        </div>
      ) : error ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-white/[0.01] rounded-2xl border border-white/[0.04] text-center p-6">
          ...
          <h3 className="font-bold text-white text-base">Learning Queue Empty</h3>
              ...
              return (
                <div 
                  ...
                  className={`rounded-xl overflow-hidden border transition-all duration-200 ${
                    isExpanded 
                      ? 'bg-slate-900/90 border-amber-500/30 shadow-[0_4px_20px_-5px_rgba(245,158,11,0.08)]' 
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className="text-white font-semibold text-sm font-mono tracking-tight truncate">
                  ...
                  {isExpanded && (
                    <div className="border-t border-white/[0.06] p-5 space-y-6 bg-slate-950/40">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-3 text-sm text-slate-300 leading-relaxed font-medium">
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className="w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-xs font-medium text-white focus:border-amber-500 focus:outline-none"
                            >
                        ...
                        <div className="space-y-4">
                          <div>
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">Proposed Intent Name</h4>
                            <input
                              type="text"
                              value={editIntent}
                              onChange={(e) => setEditIntent(e.target.value)}
                              className="w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-mono focus:border-amber-500 focus:outline-none"
                      ...
                      <div className="space-y-2">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Clause Template Draft</h4>
                        <textarea
                          rows={6}
                          value={editClause}
                          onChange={(e) => setEditClause(e.target.value)}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3.5 text-xs text-slate-200 font-mono leading-relaxed focus:border-amber-500 focus:outline-none"
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between border-b border-white/[0.06] pb-2">
                          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Variables Schema</h4>
                          <button
                            type="button"
                            ...
                            {editVars.map((v, idx) => (
                              <div key={idx} className="flex items-center gap-2 bg-white/[0.01] border border-white/[0.06] p-2.5 rounded-lg">
                                <input
                                  ...
                                  className="flex-1 bg-slate-900 border border-white/10 rounded px-2.5 py-1.5 text-xs text-white font-mono focus:border-amber-500 focus:outline-none"
                                  placeholder="variable_name"
                                />
                                <select
                                  value={v.type}
                                  onChange={(e) => handleVarChange(idx, 'type', e.target.value)}
                                  className="w-28 bg-slate-900 border border-white/10 rounded px-2 py-1.5 text-xs text-white focus:border-amber-500 focus:outline-none"
                                >
                      ...
                      <div className="flex flex-wrap items-center justify-between border-t border-white/[0.06] pt-4 gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            disabled={isItemLoading}
                            onClick={() => handleToggleExpand(item)}
                            className="text-xs font-semibold text-slate-400 hover:text-slate-300 rounded-lg px-4 py-2 hover:bg-white/5 transition-all"
                          >
                            Cancel
                          </button>
                          <button
